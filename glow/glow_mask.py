...
class Glow(nn.Module):
    ...
    def modify_mask(self):
        # print("untrained!!!!!!!!!!!!!!!1")
        # for i in range(self.mask.size(2)):
        #     print(self.mask[0, 0, i, :])
        #     break
        ...
        for i_idx in range(8):
            for j_idx in range(8):
                mean = float(mask[:, :, i_idx * 8:i_idx * 8 + 8, j_idx * 8: j_idx * 8 + 8].sum()) / (64 * 3)
                # mean = mean.expand(1, 3, 8, 8)
                # print(i_idx * 8, i_idx * 8 + 8, j_idx * 8, j_idx * 8 + 8,
                #       mean, mask[:, :, i_idx * 8:i_idx * 8 + 8, j_idx * 8: j_idx * 8 + 8].size())
                mask[:, :, i_idx * 8:i_idx * 8 + 8, j_idx * 8: j_idx * 8 + 8] = \
                    mean
        # #
        # # for i in range(mask.size(2)):
        # #     print(mask[0, 0, i, :])
